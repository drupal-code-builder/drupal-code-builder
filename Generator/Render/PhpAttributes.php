...
class PhpAttributes {
  ...
  /**
   * Creates a new attribute for a nested attribute.
   */
  public static function nested($attribute_class_name, $data) {
   ...
   *   The nesting level.
   */
  public function renderNestedAttribute(&$lines, $declaration_line, $nesting): void {
    $class_name_prefix = str_starts_with($this->attributeClassName, '\\') ? '' : '\\';
  ...
  public function renderArray(&$lines, $data, $nesting = 0, $attribute_nesting = FALSE): void {
    ...
    foreach ($data as $key => $value) {
      ...
      elseif (is_object($value)) {
        $value->renderNestedAttribute($lines, $declaration_line, $nesting);
      }
